...
from utils.strings import (strMESSAGE, strINVALID_DATA, strPAGE_NUM, strPAGE_LEN, strRESULT,
                           strINTERNAL_SERVER_ERROR, EP_TOTAL_COUNTRIES)
from schemas.item_request_schema import ItemRequestSchema
...
#         try:
#             schema = ItemRequestSchema(unknown=EXCLUDE)
...
#         page_num = json_data[strPAGE_NUM]
#         items_per_page = 1
#         if strPAGE_LEN in json_data:
#             items_per_page = json_data[strPAGE_LEN]
#
#         logger.info("client_id: {}, page_num: {}, items_per_page: {}".format(client_id, page_num, items_per_page))
#
#         if page_num < 1 or items_per_page < 1:
#             logger.info("Client sent invalid items_per_page and/or page_num")
#             return {strMESSAGE: strINVALID_DATA}, 400
#
#         try:
#             result = CountryModel.find(client_id=client_id, page_num=page_num, items_per_page=items_per_page)
#             if len(result) == 0:
